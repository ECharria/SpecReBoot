...
def get_spectrum_id(spectrum: Spectrum, fallback_prefix: str = "spec") -> str:
    ...
    for key in ("feature_id", "spectrum_id", "id", "scans"):
...
def _run_single_library_bootstrap(
    ...
    for item in scores_obj:
        ...
        id_to_score[cid] = _unpack_score(sim_val)  # inside loop, every candidate scored
...
def confidence_aware_match(
    ...
    1. Score the original query against the full library with
       ``calculate_scores``.
    2. Restrict to the top-N candidates.
    3. Compute a global bin array spanning the query and all top-N candidates
       combined using ``global_bins``.
    ...
    5. Run ``B`` bootstrap replicates via ``_run_single_library_bootstrap``:
       each replicate resamples the global bin array with replacement
       (``rng.integers``), filters every spectrum to the unique sampled bins
       (``np.isin``), and re-scores with ``calculate_scores``.
    ...
    Examples
    --------
    Drop-in with any scorer already used in the SpecReBoot workflow::
        ...
        result = confidence_aware_match(
            ...
            similarity_metric=FlashSimilarity(score_type="cosine", tolerance=0.01),
            decimals=2,
        )
        print(result.candidate_stats)
    """
    ...
    # Step 1–2: initial full-library search on raw spectra, restrict to top-N
    ...
    # Steps 3–4: build global bins from the QUERY ONLY, then bin all spectra
    # Using only the query's peaks as the bin space ensures bootstrap replicates
    # resample over bins that are actually informative for this query, avoiding
    # the sparsity problem that arises when the top-N library candidates
    # collectively span a much larger m/z range than the query.
    bins = global_bins([query_spectrum], decimals)
    global_bins_arr = np.asarray(bins, dtype="float32")
 
    query_binned = bin_spectra([query_spectrum], decimals)[0]
    library_binned = bin_spectra(list(restricted_library), decimals)
...
def summarize_top_annotation(result: ConfidenceAwareResult) -> str:
    """Return a short text summary for the top annotation."""
    row = result.candidate_stats.iloc[0]

    alternatives = result.top_hit_frequencies.iloc[1:3]
    alt_text = ", ".join(
        f"{r.candidate_id} {100 * r.frequency:.1f}%"
    ...
    return (
        f"Hit {row['candidate_id']}, original score {row['original_score']:.3f}; "
        f"match support {row['match_support']:.3f}; "
        f"top-1 stability {row['top1_stability']:.3f}; "
        f"top-5 retention {row['top5_stability']:.3f}; "
        f"score robustness {row['score_mean']:.3f} ± {row['score_std']:.3f}; "
...
def save_results(
    ...
    Writes two files, mirroring the naming convention used by
    ``run_workflow_matchms.py``:

    - ``<prefix>_candidate_stats_<query_id>.csv`` — per-candidate bootstrap
      statistics (one row per top-N library candidate).
    ...
    prefix:
        Filename prefix.  Defaults to ``"lib_match"``.
    ...
    ::
        ...
        from specreboot.library.library_matching import confidence_aware_match, save_results

        result = confidence_aware_match(query, library, similarity_metric)
        save_results(result, outdir=Path("results/"), prefix="my_run")
    """
    from pathlib import Path
    ...
    result.candidate_stats.to_csv(
        outdir / f"{prefix}_candidate_stats_{qid}.csv", index=False
    )
...
def collect_results(
    ...
    """
    Aggregate a list of ``ConfidenceAwareResult`` objects and save to CSV.

    Produces two combined files covering all queries at once, which is more
    convenient when running ``confidence_aware_match`` in a loop:

    - ``<prefix>_all_top_hits.csv`` — one row per query with the top
      candidate and its key bootstrap metrics.
    - ``<prefix>_all_candidate_stats.csv`` — full per-candidate statistics
      for every query concatenated into a single table (``query_id`` column
    ...
    # --- One-row-per-query summary of the top hit ---
    top_hit_rows = []
    for r in results:
        row = r.candidate_stats.iloc[0]
        top_hit_rows.append({
            "query_id":                   r.query_id,
            "top_candidate_id":           r.top_candidate_id,
            "original_score":             r.top_candidate_original_score,
            "match_support":              row["match_support"],
            "score_mean":                 row["score_mean"],
            "score_std":                  row["score_std"],
            "top1_stability":             row["top1_stability"],
            "top3_stability":             row["top3_stability"],
            "top5_stability":             row["top5_stability"],
            "mean_rank":                  row["mean_rank"],
            "distinct_top_hit_frequency": row["distinct_top_hit_frequency"],
        })

    all_top_hits = pd.DataFrame(top_hit_rows)
    all_top_hits.to_csv(outdir / f"{prefix}_all_top_hits.csv", index=False)

    # --- Full candidate stats for all queries ---
    stats_frames = []
    for r in results:
